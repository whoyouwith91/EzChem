...
from utils_functions import floating_type, get_n_params, option_solver, _get_index_from_matrix
...
class PhysModule(torch.nn.Module):
    ...
    def forward(self, input_dict):
        ...
        return {"vi": tmp_res, "out": out_res, "regularization": regularization}
...
if __name__ == '__main__':
    physnet = PhysModule(160, 64, 2, 1, 1, 1, "shifted_soft_plus", "none", 1, False, False).double().cuda()
    x_rand = torch.rand(torch.Size([300, 160])).double().cuda()
    edge_index_rand = _get_index_from_matrix(299, 0).long().cuda()
    edge_attr_rand = torch.rand(torch.Size([edge_index_rand.shape[-1], 64])).double().cuda()
    for i in range(11):
        out = physnet({"vi": x_rand, "edge_index": edge_index_rand, "edge_attr": edge_attr_rand})
