...
def main():
    ...
    valid_raw = pd.read_csv(os.path.join(this_dic['data_path']), 'valid.csv')
    test_raw = pd.read_csv(os.path.join(this_dic['data_path']), 'test.csv')
    ...
    for file_ in glob.iglob(os.path.join(this_dic['data_path'], 'train*')):
        ...
        if this_dic['model'] not in ['loopybp', 'wlkernel']:
            ...
            torch.save(examples, os.path.join(this_dic['save_path'], args.dataset, 'graphs', args.style, this_dic['model'], 'shard_'+str(shard_id), 'raw', +'_temp.pt')) ###
